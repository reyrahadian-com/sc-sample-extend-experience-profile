define(["sitecore", "/-/speak/v1/experienceprofile/DataProviderHelper.js", "/-/speak/v1/experienceprofile/CintelUtl.js"], function (sc, providerHelper, cintelUtil) {
	...
	var app = sc.Definitions.App.extend({
		initialized: function () {
			this.processTabs();
			sc.trigger("CustomTabApp", this);
		},

		loadPanel: function () {
			//TODO: remove tabId from function

			var panelId = $("[data-sc-id='CustomTabTabControl'] > .tab-content > .active .sc-load-on-demand-panel").data("sc-id");
		...
		showDefaultTab: function () {
			var firstTabId = this.CustomTabTabControl.get(selectedTabProperty);
			var urlTabId = this.getTabIdFromUrl();

			if (urlTabId && urlTabId != firstTabId) {
				this.CustomTabTabControl.set(selectedTabProperty, urlTabId);
			} else {
				this.loadPanel();
			}
		},
		...
		processTabs: function () {
			this.CustomTabTabControl.on("change:" + selectedTabProperty, function (tabControl, selectedTab) {
				this.loadPanel();
			}, this);
